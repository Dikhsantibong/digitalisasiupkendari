...
export default function PdmJadwalProgram5s5rIndex({
    unit,
    filters,
    options,
    days,
...
return;
        ...
        setDirty(true);
        toast.success(`Kegiatan ${targetRow.uraian} berhasil dihapus.`);
    };
...
return;
        ...
        setNewTarget(4);
        setDirty(true);
        toast.success(`Kegiatan 5S5R "${newUraian}" berhasil ditambahkan.`);
    };
    ...
    const handleSave = () => {
        ...
        router.post(
            ...
            },
            {
                ...
                onSuccess: () => {
                    setDirty(false);
                    toast.success('Jadwal Pelaksanaan 5S5R PdM Pembangkit berhasil disimpan.');
                },
                onError: () => {
...
return null;
        ...
        mergeCells(ws, headerTop, 1, headerBottom, 1); // RENCANA / REALISASI
        ...
        await buildDocumentHeader(
            { workbook, worksheet: ws },
            {
                ...
                barTitle: `JADWAL PELAKSANAAN 5S5R PdM PEMBANGKIT - ${month_name.toUpperCase()} ${filters.year}`,
            },
        );

        await downloadWorkbook(
            workbook,
            `Jadwal_Pelaksanaan_5S5R_PdM_${unit.name.replace(/\s+/g, '_')}_${filters.month}_${filters.year}.xlsx`,
        );
    };
    ...
    return (
        <>
            <Head title={`Jadwal Pelaksanaan 5S5R PdM - ${unit.name}`} />
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 no-print">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-xl font-bold tracking-tight text-foreground">
                                Jadwal Pelaksanaan 5S5R PdM Pembangkit
                            </h1>
                        </div>
                        <p className="mt-0.5 text-xs text-muted-foreground pl-10">
                            ...
                            • Ringkas, Rapi, Resik, Rawat, Rajin di Lingkungan PdM
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Button
                            ...
                            onClick={() =>
                                window.open(
                                    `/pdm/jadwal/program-5s-5r/pdf?unit_id=${filters.unit_id}&month=${filters.month}&year=${filters.year}`,
                                    '_blank',
                                )
                            }
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4 no-print">
                    <Card className="py-2.5 px-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">Total Kegiatan 5S5R</span>
                        </div>
                        <div className="mt-1 text-lg font-bold">{stats.totalItems} Uraian</div>
                    </Card>
                    <Card className="py-2.5 px-3">
                        <div className="flex items-center gap-2">
                ...
                <div className="rounded-md border border-border bg-card shadow-xs overflow-hidden">
                    ...
                    <div className="border-b border-border p-4 text-center bg-muted/10">
                        ...
                        <div className="text-base font-bold uppercase tracking-tight text-foreground mt-0.5">
                            Jadwal Pelaksanaan 5S5R PdM Pembangkit - {month_name} {filters.year}
                        </div>
                    </div>
                        ...
                        <table className="w-full min-w-[1550px] border-collapse text-xs">
                            <thead>
                                <tr className="border-b border-border bg-[#ed7d31] text-white">
                                    <th rowSpan={2} className="w-10 min-w-10 border-r border-[#d96a20] px-2 py-2 text-center font-bold">
                                        No.
                                    </th>
                                    <th rowSpan={2} className="w-[280px] min-w-[280px] border-r border-[#d96a20] px-3 py-2 text-left font-bold">
                                        URAIAN
                                    </th>
                                    <th rowSpan={2} className="w-24 min-w-24 border-r border-[#d96a20] px-2 py-2 text-center font-bold">
                                        RENCANA / REALISASI
                                    </th>
                                    {days.map((d) => (
                                        <th
                            ...
                            <tbody>
                                {rows.map((row, rIdx) => {
                                    ...
                                    return (
                                        <Fragment key={`item-${rIdx}`}>
                                            {/* Sub-row 1: RENCANA */}
                                            <tr className="border-b border-border/40 hover:bg-muted/10 transition-colors">
                                                {/* No Urut (rowSpan 2) */}
                                                <td rowSpan={2} className="border-r border-border px-1 py-1.5 text-center text-muted-foreground font-medium w-10 min-w-10">
                                                    {row.no_urut}
                                                </td>

                                                {/* Uraian (rowSpan 2) */}
                                                <td rowSpan={2} className="border-r border-border px-2 py-1 font-semibold text-foreground w-[280px] min-w-[280px]">
                                                    {can_write ? (
                                                        <input
                                                            type="text"
                                                            value={row.uraian}
                                                            onChange={(e) => handleUpdateUraian(rIdx, e.target.value)}
                                                            placeholder="Uraian kegiatan 5S5R..."
                                                ...
                                                {/* Label RENCANA */}
                                                <td className="border-r border-border px-2 py-1 text-center font-bold text-[11px] text-amber-700 dark:text-amber-400 bg-amber-50/50 dark:bg-amber-950/20 w-24 min-w-24">
                                                    RENCANA
                                                </td>
                                            ...
                                            <tr className="border-b border-border hover:bg-muted/10 transition-colors">
                                                {/* Label REALISASI */}
                                                <td className="border-r border-border px-2 py-1 text-center font-bold text-[11px] text-emerald-700 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-950/20 w-24 min-w-24">
                                                    REALISASI
                                                </td>
            ...
            <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-base font-bold">Tambah Kegiatan 5S5R</DialogTitle>
                        <DialogDescription className="text-xs">
                            Masukkan uraian kegiatan 5S5R dan target pelaksanaan dalam satu bulan.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 py-2 text-xs">
                        <div>
                            ...
                            <Input
                                ...
                                placeholder="Contoh: RINGKAS (SEIRI)..."
            ...
            <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs">
                            Perbarui nomor urut, uraian kegiatan, atau target pelaksanaan.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 py-2 text-xs">
...
PdmJadwalProgram5s5rIndex.layout = {
    breadcrumbs: [
        { title: 'Dashboard', href: dashboard() },
        { title: 'Jadwal PdM & Maturity Level', href: '/pdm/jadwal' },
        { title: 'Jadwal Pelaksanaan 5S5R PdM Pembangkit', href: '/pdm/jadwal/program-5s-5r' },
    ],
};
