...
import instruksiKerja from '@/routes/k3/jadwal/instruksi-kerja';
...
type TextField = 'kegiatan' | 'waktu' | 'pic' | 'peserta' | 'keterangan';
...
export default function K3InstruksiKerjaPage({
    unit,
    filters,
    options,
    days,
    ...
    const [form, setForm] = useState({ kegiatan: '', waktu: '08:00-08:30', pic: '', peserta: 'seluruh Personel' });
    ...
    const updateField = (key: number, field: TextField, value: string) => {
        if (!can_write) return;
        setRows((prev) =>
    ...
    const handleAdd = () => {
        ...
        setDirty(true);
        setIsAddOpen(false);
        setForm({ kegiatan: '', waktu: '08:00-08:30', pic: '', peserta: 'seluruh Personel' });
    };
    ...
    const textCell = (row: Row, field: TextField, extra = '') =>
        can_write ? (
            <input
                type="text"
                value={row[field] || ''}
                onChange={(e) => updateField(row._key, field, e.target.value)}
                className={`w-full bg-transparent px-1 py-0.5 text-xs focus:rounded focus:bg-background focus:ring-1 focus:ring-primary focus:outline-none ${extra}`}
            />
        ) : (
            <span className="px-1 text-xs">{row[field] || '-'}</span>
        );
    ...
    const dayCells = (row: Row, category: Category) =>
        days.map((d) => {
            ...
            const isOn = row[category].includes(d.day);
            return (
                <td
                    key={`${category}-${d.day}`}
                    onClick={() => toggleCell(row._key, category, d.day)}
                    className={`border border-black text-center text-[11px] font-bold transition-colors select-none ${
                        isOn ? 'bg-slate-200 text-foreground dark:bg-slate-700' : 'hover:bg-muted/40'
                    } ${can_write ? 'cursor-pointer' : ''}`}
                    title={`Tanggal ${d.day} (${d.dow}) — ${category === 'rencana' ? 'Rencana' : 'Realisasi'}: klik untuk ${isOn ? 'batalkan' : 'tandai (1)'}`}
                >
                    {isOn ? '1' : ''}
                </td>
            );
    ...
    return (
        <>
            <Head title={`Jadwal Instruksi Kerja K3L — ${unit.name}`} />
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
                ...
                <div className="no-print flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                                Klik kotak tanggal pada baris RENC (rencana) atau REAL (realisasi). Target = jumlah rencana, kinerja = realisasi ÷ target.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <a
                            href={`${instruksiKerja.pdf().url}?unit_id=${filters.unit_id}&month=${filters.month}&year=${filters.year}`}
                            target="_blank"
                            rel="noreferrer"
                        >
                    ...
                    <div className="border-b border-border p-4">
                        <div className="grid grid-cols-12 items-stretch border border-black dark:border-border">
                            ...
                            <div className="col-span-6 flex flex-col justify-center p-2 text-center">
                                <div className="text-xs font-bold tracking-wide text-foreground uppercase">JASA PENDUKUNG TEKNIS UP KENDARI - {unit.name.toUpperCase()}</div>
                                <div className="mt-0.5 text-[11px] font-semibold text-foreground uppercase">LAPORAN PROJECT</div>
                                <div className="text-[11px] font-bold text-foreground uppercase">JADWAL INSTRUKSI KERJA K3 DAN LINGKUNGAN — {monthName.toUpperCase()} {filters.year}</div>
                            </div>
                        ...
                        <table className="print-table w-full border-collapse text-xs">
                            <thead className="print-thead bg-muted/60 dark:bg-muted/30">
                                <tr>
                                    ...
                                    <th rowSpan={2} className="w-56 min-w-48 border border-black p-1 text-left font-bold text-foreground">Kegiatan</th>
                                    <th rowSpan={2} className="w-20 border border-black p-1 text-center font-bold text-foreground">Waktu</th>
                                    <th rowSpan={2} className="w-16 border border-black p-1 text-center font-bold text-foreground">PIC</th>
                                    <th rowSpan={2} className="w-20 border border-black p-1 text-center font-bold text-foreground">Peserta</th>
                                    <th rowSpan={2} className="w-12 border border-black p-1 text-center font-bold text-foreground">Status</th>
                                    <th colSpan={days.length} className="border border-black p-1 text-center font-bold tracking-wider text-foreground uppercase">Timeline</th>
                                    ...
                                    return (
                                        <Fragment key={row._key}>
                                            ...
                                            <tr className="transition-colors hover:bg-muted/10">
                                                ...
                                                <td rowSpan={2} className="border border-black p-1 text-left align-top text-foreground">{textCell(row, 'kegiatan')}</td>
                                                <td rowSpan={2} className="border border-black p-1 text-center align-top">{textCell(row, 'waktu', 'text-center')}</td>
                                                <td rowSpan={2} className="border border-black p-1 text-center align-top">{textCell(row, 'pic', 'text-center')}</td>
                                                <td rowSpan={2} className="border border-black p-1 text-center align-top">{textCell(row, 'peserta', 'text-center')}</td>
                                                ...
                                                <td rowSpan={2} className="border border-black p-1 text-left align-top">{textCell(row, 'keterangan')}</td>
                                                {can_write && (
                                                    <td rowSpan={2} className="no-print border border-black p-1 text-center align-middle">
                                                        <Button variant="ghost" size="icon" className="size-6 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={() => handleRemove(row._key)} title="Hapus Kegiatan">
                    ...
                    <div className="border-t border-border bg-muted/20 p-4">
                        <div className="flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex flex-wrap items-center gap-4">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <span className="font-semibold text-foreground">RENC</span>
                                    <span>= Rencana ·</span>
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span>Target otomatis dari jumlah rencana; kinerja = realisasi ÷ target.</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Tambah Instruksi Kerja K3L
                        </DialogTitle>
                        <DialogDescription>
                            Tambahkan kegiatan instruksi kerja baru untuk unit {unit.name}.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="grid gap-3 py-2 text-sm">
                        <div className="grid gap-1.5">
                            <Label htmlFor="ik-kegiatan">Nama Kegiatan</Label>
                            <Input id="ik-kegiatan" placeholder="Contoh: Safety Briefing" value={form.kegiatan} onChange={(e) => setForm((f) => ({ ...f, kegiatan: e.target.value }))} autoFocus />
                        </div>
                        <div className="grid gap-1.5">
                            <Label htmlFor="ik-waktu">Waktu</Label>
                            <Input id="ik-waktu" placeholder="08:00-08:30" value={form.waktu} onChange={(e) => setForm((f) => ({ ...f, waktu: e.target.value }))} />
                        </div>
                        <div className="grid gap-1.5">
                            <Label htmlFor="ik-pic">PIC</Label>
                            <Input id="ik-pic" placeholder="(opsional)" value={form.pic} onChange={(e) => setForm((f) => ({ ...f, pic: e.target.value }))} />
                        </div>
                        <div className="grid gap-1.5">
                            <Label htmlFor="ik-peserta">Peserta</Label>
                            <Input id="ik-peserta" placeholder="seluruh Personel / PIC K3L" value={form.peserta} onChange={(e) => setForm((f) => ({ ...f, peserta: e.target.value }))} />
                        </div>
                    </div>

                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsAddOpen(false)}>Batal</Button>
                        <Button onClick={handleAdd} disabled={!form.kegiatan.trim()}>Tambahkan</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
...
K3InstruksiKerjaPage.layout = {
    breadcrumbs: [
        { title: 'Dashboard', href: dashboard() },
        { title: 'Jadwal K3 & Lingkungan', href: jadwal.index() },
        { title: 'Instruksi Kerja K3L', href: instruksiKerja.index() },
    ],
};
