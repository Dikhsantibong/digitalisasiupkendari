...
type ServerRow = { id: number | null; uraian: string; shift: string; rencana: number[]; realisasi: number[]; target: number };
...
export default function InventarisPage({ unit, filters, options, days_in_month, rows: initial, can_write }: Props) {
    ...
    const visit = (p: Partial<Props['filters']>) => router.get(inventaris.index().url, { ...filters, ...p }, { preserveState: true, preserveScroll: true, replace: true });
    const toggle = (key: number, cat: Cat, day: number) => { if (!can_write) return; setRows((prev) => prev.map((r) => { if (r._key !== key) return r; const s = new Set(r[cat]); s.has(day) ? s.delete(day) : s.add(day); return { ...r, [cat]: Array.from(s).sort((a, b) => a - b) }; })); setDirty(true); };
    const setField = (key: number, f: 'uraian' | 'shift' | 'target', v: string) => { setRows((p) => p.map((r) => (r._key === key ? { ...r, [f]: f === 'target' ? Math.max(0, parseInt(v, 10) || 0) : v } : r))); setDirty(true); };
    const addRow = () => { setRows((p) => [...p, { _key: nextKey, id: null, uraian: '', shift: 'JADWAL', rencana: [], realisasi: [], target: 5 }]); setNextKey((k) => k + 1); setDirty(true); };
    ...
    const save = () => { if (!can_write) return; setSaving(true); router.post(inventaris.store().url, { unit_id: filters.unit_id, month: filters.month, year: filters.year, rows: rows.map((r) => ({ id: r.id, uraian: r.uraian || r.shift || 'Inventarisasi', shift: r.shift, rencana: r.rencana, realisasi: r.realisasi, target: r.target })) }, { preserveScroll: true, onSuccess: () => setDirty(false), onFinish: () => setSaving(false) }); };

    const handleExcel = async () => {
        const H: (string | number)[][] = [[], [], [], []];
        H.push(['URAIAN', 'SHIFT', 'STATUS', ...days.map(String), 'RENCANA', 'TARGET', 'REALISASI', 'A. KINERJA']);
        rows.forEach((r) => {
            const kin = r.target > 0 ? Math.round((r.realisasi.length / r.target) * 100) : 0;
            H.push([r.uraian, r.shift, 'RENCANA', ...days.map((d) => (r.rencana.includes(d) ? 1 : '')), r.rencana.length, r.target, r.realisasi.length, `${kin}%`]);
            H.push(['', '', 'REALISASI', ...days.map((d) => (r.realisasi.includes(d) ? 1 : '')), '', '', '', '']);
        });
        const { workbook, worksheet: ws } = createSheet('Inventarisasi', H);
        const dayStart = 3; const dayEnd = dayStart + days.length - 1; const colRenc = dayStart + days.length; const colKinerja = colRenc + 3; const totalCols = colKinerja + 1;
        ...
        paintSheet(ws, H.length, totalCols, (r, c) => {
            ...
            if (c === 2) return SPECS.labelCenter;
            if (c >= dayStart && c <= dayEnd) return H[r]?.[c] === 1 ? done : SPECS.cell;
            return { ...SPECS.cell, bold: true };
        });
        rows.forEach((_, i) => { const top = 5 + i * 2; mergeCells(ws, top, 0, top + 1, 0); mergeCells(ws, top, 1, top + 1, 1); for (let c = colRenc; c <= colKinerja; c++) mergeCells(ws, top, c, top + 1, c); });
        const colWidths = [26, 8, 12, ...Array(days.length).fill(3.2), 9, 8, 9, 11];
        setColWidths(ws, colWidths);
        await buildDocumentHeader({ workbook, worksheet: ws }, { totalCols, colWidths, titleLines: ['JASA PENDUKUNG TEKNIS 11 & 6 SITE - KIT', `LAPORAN PROJECT — ${unit.name.toUpperCase()}`, 'INVENTARISASI TOOLS & MATERIAL'], barTitle: `JADWAL INVENTARISASI TOOLS DAN MATERIAL OPERASI — ${monthName.toUpperCase()} ${filters.year}` });
        await downloadWorkbook(workbook, `Jadwal_Inventarisasi_Tools_${unit.name.replace(/\s+/g, '_')}_${filters.month}_${filters.year}.xlsx`);
    };

    const th = 'p-th border border-black bg-[#ea7315] p-1 text-center text-[10px] font-bold text-white';
    const dayCells = (r: Row, cat: Cat) => days.map((d) => { const on = r[cat].includes(d); return <td key={`${cat}-${d}`} onClick={() => toggle(r._key, cat, d)} className={`border border-black text-center text-[11px] font-bold select-none ${on ? 'p-done bg-slate-300 dark:bg-slate-600' : 'hover:bg-muted/40'} ${can_write ? 'cursor-pointer' : ''}`}>{on ? '1' : ''}</td>; });

    return (
        <>
            <Head title={`Jadwal Inventarisasi Tools & Material — ${unit.name}`} />
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
                <div className="no-print flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div><div className="flex items-center gap-2"><h1 className="text-xl font-bold text-foreground">Jadwal Inventarisasi Tools &amp; Material Operasi</h1><Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">{unit.name}</Badge></div><p className="text-xs text-muted-foreground">Klik sel RENCANA / REALISASI untuk mengisi 1. Kinerja = realisasi ÷ target.</p></div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {can_write && <><Button variant="outline" size="sm" onClick={markPlan} className="gap-1.5 text-xs"><CheckCheck className="size-3.5 text-primary" />Tandai Rencana</Button><Button variant="outline" size="sm" onClick={addRow} className="gap-1.5 text-xs"><Plus className="size-3.5" />Tambah</Button><Button size="sm" onClick={save} disabled={saving || !dirty} className="gap-1.5 text-xs"><Save className="size-3.5" />{saving ? 'Menyimpan…' : 'Simpan'}</Button></>}
                        ...
                        <a href={`${inventaris.pdf().url}?unit_id=${filters.unit_id}&month=${filters.month}&year=${filters.year}`} target="_blank" rel="noreferrer"><Button variant="outline" size="sm" className="gap-1.5 text-xs"><Download className="size-3.5 text-rose-600" />PDF</Button></a>
                ...
                <div className="print-container rounded-md border border-border bg-card">
                    <div className="border-b border-border p-3">
                        <div className="grid grid-cols-12 items-stretch border border-black dark:border-border">
                            ...
                            <div className="col-span-6 flex flex-col justify-center p-2 text-center">
                                ...
                                <div className="text-[11px] font-bold text-foreground uppercase">JADWAL INVENTARISASI TOOLS DAN MATERIAL OPERASI — {monthName.toUpperCase()} {filters.year}</div>
                            </div>
                    ...
                    <table className="p-table w-full border-collapse text-xs">
                        <thead>
                            <tr>
                                <th rowSpan={2} className={`${th} min-w-40 text-left`}>Uraian</th>
                                <th rowSpan={2} className={th}>Shift</th>
                                ...
                                return (
                                    <Fragment key={r._key}>
                                        <tr className="hover:bg-muted/10">
                                            <td rowSpan={2} className="border border-black p-1 text-left align-middle">{can_write ? <input value={r.uraian} onChange={(e) => setField(r._key, 'uraian', e.target.value)} className="w-full bg-transparent px-1 py-0.5 text-xs font-semibold focus:rounded focus:bg-background focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Uraian…" /> : <span className="text-xs font-semibold">{r.uraian}</span>}</td>
                                            <td rowSpan={2} className="border border-black p-0 text-center align-middle">{can_write ? <input value={r.shift} onChange={(e) => setField(r._key, 'shift', e.target.value)} className="h-7 w-16 bg-transparent text-center text-xs font-semibold focus:bg-background focus:outline-none focus:ring-1 focus:ring-primary" /> : <span className="text-xs font-semibold">{r.shift}</span>}</td>
...
InventarisPage.layout = {
    breadcrumbs: [
        { title: 'Dashboard', href: dashboard() },
        { title: 'Jadwal Operasi', href: jadwal.index() },
        { title: 'Jadwal Inventarisasi Tools & Material', href: inventaris.index() },
    ],
};
