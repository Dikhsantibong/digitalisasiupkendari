...
export default function DataTeknisPage({ unit, filters, options, rows: initial, can_write }: Props) {
    ...
    const visit = (p: Partial<Props['filters']>) => router.get(dataTeknis.index().url, { ...filters, ...p }, { preserveState: true, preserveScroll: true, replace: true });
    ...
    const save = () => { if (!can_write) return; setSaving(true); router.post(dataTeknis.store().url, { unit_id: filters.unit_id, year: filters.year, rows: rows.map((r) => ({ id: r.id, nama: r.nama, pic: r.pic, months: r.months })) }, { preserveScroll: true, onSuccess: () => setDirty(false), onFinish: () => setSaving(false) }); };

    const handleExcel = async () => {
        const H: (string | number)[][] = [[], [], [], []];
        H.push(['NO', 'DATA TEKNIS', 'PIC PEMBUAT', ...MONTHS, 'JUMLAH']);
        rows.forEach((r, i) => H.push([i + 1, r.nama, r.pic, ...MONTHS.map((_, mi) => (r.months[String(mi + 1)] ? 1 : '')), jumlahOf(r)]));
        const { workbook, worksheet: ws } = createSheet('Data_Teknis', H);
        ...
        await buildDocumentHeader({ workbook, worksheet: ws }, { totalCols, colWidths, titleLines: ['JASA PENDUKUNG TEKNIS 11 & 6 SITE - KIT', `LAPORAN PROJECT — ${unit.name.toUpperCase()}`, 'JADWAL PEMBUATAN DATA TEKNIS'], barTitle: `JADWAL PEMBUATAN DATA TEKNIS PEMBANGKIT — TAHUN ${filters.year}` });
        await downloadWorkbook(workbook, `Jadwal_Pembuatan_Data_Teknis_${unit.name.replace(/\s+/g, '_')}_${filters.year}.xlsx`);
    };
    ...
    return (
        <>
            <Head title={`Jadwal Pembuatan Data Teknis — ${unit.name}`} />
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
                <div className="no-print flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div><div className="flex items-center gap-2"><h1 className="text-xl font-bold text-foreground">Jadwal Pembuatan Data Teknis Pembangkit</h1><Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">{unit.name}</Badge></div><p className="text-xs text-muted-foreground">Klik sel bulan untuk menandai target pembuatan dokumen (1). JUMLAH = total bulan terisi.</p></div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {can_write && <><Button variant="outline" size="sm" onClick={addRow} className="gap-1.5 text-xs"><Plus className="size-3.5" />Tambah Data</Button><Button size="sm" onClick={save} disabled={saving || !dirty} className="gap-1.5 text-xs"><Save className="size-3.5" />{saving ? 'Menyimpan…' : 'Simpan'}</Button></>}
                        ...
                        <a href={`${dataTeknis.pdf().url}?unit_id=${filters.unit_id}&year=${filters.year}`} target="_blank" rel="noreferrer"><Button variant="outline" size="sm" className="gap-1.5 text-xs"><Download className="size-3.5 text-rose-600" />PDF</Button></a>
                    ...
                    <div className="border-b border-border p-3">
                        <div className="grid grid-cols-12 items-stretch border border-black dark:border-border">
                            ...
                            <div className="col-span-6 flex flex-col justify-center p-2 text-center">
                                ...
                                <div className="text-[11px] font-bold text-foreground uppercase">JADWAL PEMBUATAN DATA TEKNIS PEMBANGKIT — TAHUN {filters.year}</div>
                            </div>
                    ...
                    <table className="p-table w-full border-collapse text-xs">
                        <thead>
                            <tr>
                                ...
                                <th rowSpan={2} className={`${th} min-w-64 text-left`}>Data Teknis</th>
                        ...
                        <tbody>
                            <tr className="bg-[#ea7315]/10"><td colSpan={can_write ? 16 : 15} className="border border-black p-1 text-left text-xs font-bold uppercase">A. Pembuatan Data Teknis</td></tr>
                            {rows.map((r, i) => (
                                <tr key={r._key} className="hover:bg-muted/10">
                                    <td className="border border-black p-1 text-center">{i + 1}</td>
                                    <td className="border border-black p-1 text-left">{can_write ? <input value={r.nama} onChange={(e) => setField(r._key, 'nama', e.target.value)} className="w-full bg-transparent px-1 py-0.5 text-xs focus:rounded focus:bg-background focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Nama data teknis…" /> : <span className="text-xs">{r.nama}</span>}</td>
...
DataTeknisPage.layout = {
    breadcrumbs: [
        { title: 'Dashboard', href: dashboard() },
        { title: 'Jadwal Operasi', href: jadwal.index() },
        { title: 'Jadwal Pembuatan Data Teknis', href: dataTeknis.index() },
    ],
};
