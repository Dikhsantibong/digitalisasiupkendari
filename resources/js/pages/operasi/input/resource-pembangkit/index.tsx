...
export default function ResourcePembangkitInput({
    ...
    const totalPengiriman = rows.reduce(
        (acc, r) => acc + (r.pengiriman || 0),
        0,
    );
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 md:p-6">
                ...
                <div className="overflow-hidden rounded-lg border border-border bg-card shadow-xs">
                    ...
                    {compact ? (
                        <div className="flex flex-col gap-3 bg-muted/30 p-2">
                            <DayStrip
                                items={rows.map((row) => ({
                                    key: String(row.tanggal),
                                    label: String(row.tanggal),
                                    done:
                                        row.pemakaian > 0 || row.pengiriman > 0,
                                }))}
                                value={String(selectedDay)}
                                onChange={(key) => setSelectedDay(Number(key))}
                            />
                            {rows.map((row, index) =>
                                row.tanggal !== selectedDay ? null : (
                                    <div
                                        key={row.tanggal}
                                        className="grid grid-cols-2 gap-2.5 rounded-xl border border-border bg-card p-3"
                                    >
                                        <p className="col-span-2 text-[13px] font-semibold text-foreground">
                                            Tanggal {row.tanggal}
                                        </p>
                                        {(
                                            [
                                                ['stok_awal', 'Stok awal (L)'],
                                                ['pemakaian', 'Pemakaian (L)'],
                                                [
                                                    'pengiriman',
                                                    'Pengiriman (L)',
                                                ],
                                        ...
                                        <div className="flex flex-col gap-1 text-[12px] text-muted-foreground">
                                            Stok akhir (L)
                                            <span className="flex h-9 items-center rounded-md bg-muted/60 px-3 text-[13px] font-semibold text-foreground tabular-nums">
                                                {row.stok_akhir.toLocaleString(
                                                    'id-ID',
                                                )}
                                            </span>
                                        </div>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow className="bg-[#ea7315]/20 font-bold hover:bg-[#ea7315]/20">
                                        ...
                                        <TableCell className="text-right text-amber-600 dark:text-amber-400">
                                            {totalPemakaian.toLocaleString(
                                                'id-ID',
                                            )}
                                        </TableCell>
                                        <TableCell className="text-right text-primary">
                                            {totalPengiriman.toLocaleString(
                                                'id-ID',
                                            )}
                                        </TableCell>
                    ...
                    {can_write && (
                        <div className="flex items-center justify-between border-t border-border bg-card p-3">
                            <span className="text-xs text-muted-foreground">
                                * Nilai Stok Akhir dihitung otomatis: Stok Awal
...
ResourcePembangkitInput.layout = {
    breadcrumbs: [
        ...
        { title: 'Input Operasi', href: harInput.index() },
        {
            ...
            href: resourcePembangkitRoutes.index(),
        },
    ],
};
