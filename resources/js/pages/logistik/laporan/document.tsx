...
export default function LogistikLaporanDocument({
    filters,
    document_number,
    content,
    content_styles,
    ...
    return (
        <>
            <Head title="Dokumen Laporan Logistik & Gudang Pembangkit" />
            <DocumentEditor
                title="Dokumen Laporan Logistik & Gudang Pembangkit"
                description="Laporan penuh terisi otomatis dari jadwal & input logistik & gudang. Edit sebagai teks (seperti Word) atau spreadsheet (seperti Excel), simpan, lalu unduh PDF atau Excel."
                ...
                pdfUrl={pdf_url}
                canEdit={can_write}
                baseName={`Laporan-Logistik-${filters.unit_id}-${filters.month}-${filters.year}`}
                xlsxHeaderLines={[
                    { text: 'JASA PENDUKUNG TEKNIS UP KENDARI 11 & 6 SITE - KIT', bold: true },
                    { text: 'LAPORAN LOGISTIK & GUDANG PEMBANGKIT', bold: true },
                ]}
...
LogistikLaporanDocument.layout = {
    breadcrumbs: [
