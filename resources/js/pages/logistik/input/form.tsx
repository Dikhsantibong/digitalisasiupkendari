...
/**
 * One Logistik & Gudang table form (App\Support\LogistikForms) — Laporan
 * Pendukung, Peralatan/Material/Tools, Kondisi Stok, Unsafe Action &
 * Condition — with live computed columns & totals, photos, Simpan, PDF, Excel.
 */
export default function LogistikFormInput({ form, unit, filters, options, rows: initialRows, summary, has_saved, can_write }: Props) {
    ...
    const save = () => {
        ...
        router.post(formRoutes.store(form.key).url, { ...query, rows: payload }, {
            ...
            preserveScroll: true,
            onSuccess: (page) => {
                setRows(normalize((page.props as unknown as Props).rows));
                setUploads({});
                setDirty(false);
            },
...
LogistikFormInput.layout = (props: Props) => ({
    breadcrumbs: [
