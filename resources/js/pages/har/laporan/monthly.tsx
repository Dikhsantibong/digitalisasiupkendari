...
type Data = {
    ...
    wo_waiting: { reason: string; rows: { wonum: string; description: string | null; status: string | null }[] }[];
    cost: {
        auto_service: number;
        ...
        <>
            ...
            <div className="flex min-h-screen flex-col bg-slate-200">
                ...
                <div className="print-canvas flex-1 overflow-auto p-4 md:p-8">
                    ...
                    <h1 className="text-lg font-bold uppercase">Laporan Pemeliharaan (HAR)</h1>
                    <p className="text-sm">{data.unit.name} · Periode {data.period.label}</p>
                </header>

                <h2 className="mb-1 font-semibold">1. Service Request Summary</h2>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className="border px-2 py-1">
                                ...
                                <div>
                                    {data.sr_summary.by_category.map((c) => `${c.category}: ${c.count}`).join(' · ') || '—'}
                                </div>
                            </td>
                        </tr>
                    </tbody>
                ...
                <h2 className="mb-1 font-semibold">2. Work Order Summary</h2>
                <table className="mb-4 w-full border-collapse text-[11px]">
                    <tbody>
                        <tr>
                            <Stat label="Total WO" value={String(data.wo_summary.total)} />
                            <Stat label="Complete" value={String(data.wo_summary.complete)} />
                ...
                <h2 className="mb-1 font-semibold">3. Rekapitulasi WO per Jenis</h2>
                {data.wo_by_type.length === 0 ? (
                    <p className="mb-4 text-slate-500">Tidak ada Work Order.</p>
                ) : (
                    data.wo_by_type.map((group) => (
                ...
                <h2 className="mb-1 mt-4 font-semibold">4. WO Tertunda</h2>
                {data.wo_waiting.length === 0 ? (
                    <p className="mb-4 text-slate-500">Tidak ada WO tertunda.</p>
                ) : (
                    data.wo_waiting.map((group) => (
                        <div key={group.reason} className="mb-2">
                            <p className="font-semibold">{group.reason}</p>
                            <ul className="ml-4 list-disc">
                                {group.rows.map((r) => (
                                    <li key={r.wonum}>{r.wonum} — {r.description ?? ''} ({r.status ?? '—'})</li>
                                ))}
                            </ul>
                        </div>
                    ))
                ...
                <h2 className="mb-1 mt-4 font-semibold">5. Akumulasi Biaya Pemeliharaan</h2>
                ...
                <h2 className="mb-1 mt-4 font-semibold">6. Rencana vs Realisasi</h2>
                ...
                <h2 className="mb-1 mt-4 font-semibold">7. Log Kegiatan HARMES</h2>
                ...
                <h2 className="mb-1 mt-4 font-semibold">8. Lampiran Foto</h2>
