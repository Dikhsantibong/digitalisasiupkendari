...
type Props = {
    ...
    form_data: {
        ...
        checklist_items: CylinderChecklistItem[];
        notes: string;
...
export default function PrelubeTestPage({
    unit,
    units,
    machines,
    selected_machine_id,
    ...
    const [docNumber, setDocNumber] = useState<string>(
        form_data.document_number || 'FMKD-314-10.3.3.a-B12'
    );
    ...
    const updateChecklistItem = (
        cylinder: number,
        field: keyof CylinderChecklistItem,
        val: string
    ) => {
        setChecklistItems((prev) =>
            prev.map((item) =>
    ...
    const handleSave = (onSuccessCallback?: () => void) => {
        ...
        router.post(prelubeTestRoutes.store().url, payload, {
            preserveScroll: true,
            onSuccess: () => {
                setPreviewKey((k) => k + 1);
    ...
    return (
        <>
            <Head title={`Formulir Checklist Prelube Test - ${unit.name}`} />
                                        ...
                                        <table className="w-full border-collapse text-xs min-w-[700px]">
                                            <thead>
                                                <tr className="border-b border-border bg-muted/40 font-semibold text-center">
                                                    <th rowSpan={2} className="border-r border-border p-2 w-12">
                                                        NO
                                                    </th>
                                                    <th colSpan={4} className="border-r border-border p-2 tracking-wider">
                                                        KONDISI PELUMASAN
                                                    </th>
                                                    <th rowSpan={2} className="p-2 w-52 text-left">
                                                        KETERANGAN
                                                    </th>
                                                </tr>
                                                <tr className="border-b border-border bg-muted/25 font-semibold text-center text-[11px]">
                                                    <th className="border-r border-border p-1.5 w-28">CAM SHAFT</th>
                                                    <th className="border-r border-border p-1.5 w-28">
                                                        CRANK PIN (CONROD)
                                                    </th>
                                                    <th className="border-r border-border p-1.5 w-28">
                                                        CRANK PIN (PISTON)
                                                    </th>
                                                    <th className="border-r border-border p-1.5 w-28">ROCKER ARM</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-border">
                                                {checklistItems.map((item) => {
                                                    const renderToggleGroup = (field: 'camshaft' | 'conrod' | 'piston' | 'rocker_arm') => {
                                                        ...
                                                        return (
                                                            <div className="flex items-center justify-center gap-1">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => updateChecklistItem(item.cylinder, field, current === 'v' ? '' : 'v')}
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    onClick={() => updateChecklistItem(item.cylinder, field, current === 'X' ? '' : 'X')}
                                                                    className={`size-6 rounded flex items-center justify-center font-bold text-xs transition-colors ${
                                                                        current === 'X'
                                                                            ? 'bg-red-600 text-white shadow-sm'
                                                                            : 'bg-muted/70 text-muted-foreground hover:bg-muted'
                                                                    }`}
                                                                    title="X = Tidak Keluar Oli"
                                                                >
                                                                    X
                                                                </button>
                                                            </div>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <td className="border-r border-border p-2 text-center font-bold">
                                                                {item.cylinder}
                                                            </td>
                                                            <td className="border-r border-border p-2 text-center">
                                                                {renderToggleGroup('camshaft')}
                                                            </td>
                                                            <td className="border-r border-border p-2 text-center">
                                                                {renderToggleGroup('conrod')}
                                                            </td>
                                                            <td className="border-r border-border p-2 text-center">
                                                                {renderToggleGroup('piston')}
                                                            </td>
                                                            <td className="border-r border-border p-2 text-center">
                                                                {renderToggleGroup('rocker_arm')}
                                                            </td>
                                                            <td className="p-2">
                                                                <Input
                                                                    value={item.notes}
                                                                    onChange={(e) => updateChecklistItem(item.cylinder, 'notes', e.target.value)}
                                                                    placeholder="Catatan silinder..."
                                                                    className="h-7 text-xs"
                                                                />
                                                            </td>
                                                        </tr>
                                        ...
                                        <div className="rounded-md border border-border bg-muted/20 p-3 space-y-1.5 text-xs">
                                            ...
                                            <div className="space-y-1 text-muted-foreground">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span><strong>v</strong> = Kondisi Ada / Keluar Oli</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span className="size-5 rounded bg-red-600 text-white font-bold flex items-center justify-center text-[10px]">
                                                        X
                                                    </span>
                                                    <span><strong>X</strong> = Kondisi Tidak Ada / Tidak Keluar Oli</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="space-y-1.5 text-xs">
                                            <Label className="font-semibold">Catatan Umum / Hasil Pengujian:</Label>
                                            <textarea
                                                rows={3}
                                                value={notes}
                                                onChange={(e) => setNotes(e.target.value)}
                                                placeholder="Tulis catatan tambahan pengujian di sini..."
            ...
            <Dialog open={showHistory} onOpenChange={setShowHistory}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Riwayat Formulir Prelube Test - {unit.name}</DialogTitle>
                        <DialogDescription>
                            Daftar catatan uji prelube test yang pernah disimpan untuk unit dan mesin ini.
                        </DialogDescription>
                    </DialogHeader>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                router.get(prelubeTestRoutes.index().url, {
                                                    unit_id: unit.id,
...
PrelubeTestPage.layout = {
    breadcrumbs: [
        ...
        { title: 'Checklist Prelube Test', href: prelubeTestRoutes.index() },
    ],
};
