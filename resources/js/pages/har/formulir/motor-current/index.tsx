...
type Props = {
    ...
    history: HistoryItem[];
    pdf_url: string;
    sample_scan_items: MotorCurrentItem[];
    can_write: boolean;
};

export default function MotorCurrentIndex({
    unit,
    units,
    machines,
    selected_machine_id,
    ...
    // Motor Items & General Notes
    const [items, setItems] = useState<MotorCurrentItem[]>(form_data.items || []);
    ...
    // Update single motor item field
    const updateItemField = (
        index: number,
        field: keyof Omit<MotorCurrentItem, 'no'>,
        val: string
    ) => {
        setItems((prev) =>
    ...
    const handleFillSampleScan = () => {
        setItems(sample_scan_items);
        setGeneralNotes(
            'Pengukuran arus kerja elektro motor dilakukan saat kondisi operasi normal. Beban fasa R, S, dan T seimbang dan sesuai batas nominal motor.'
        );
    };

    // Reset items to standard 14 rows
    ...
    const previewPdfUrl = useMemo(() => {
        ...
        items.forEach((it, idx) => {
            params.set(`items[${idx}][no]`, String(it.no));
            params.set(`items[${idx}][motor_name]`, it.motor_name);
            params.set(`items[${idx}][current_r]`, it.current_r);
            params.set(`items[${idx}][current_s]`, it.current_s);
            params.set(`items[${idx}][current_t]`, it.current_t);
            params.set(`items[${idx}][notes]`, it.notes || '');
        });
    ...
    const handleSave = (onSuccessCallback?: () => void) => {
        ...
        router.post(motorCurrentRoutes.store().url, payload, {
            preserveScroll: true,
            onSuccess: () => {
                setPreviewKey((k) => k + 1);
    ...
    return (
        <>
            <Head title={`Data Pengukuran Arus Kerja Elektro Motor - ${unit.name}`} />
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
                ...
                <div className="flex flex-col lg:flex-row items-start gap-6 w-full">
                    ...
                    <div className="w-full lg:w-[380px] xl:w-[420px] shrink-0 space-y-5">
                        ...
                        <div className="rounded-lg border border-border bg-card p-4 space-y-4">
                            <div className="border-b border-border pb-2">
                                ...
                                <p className="text-[12px] text-muted-foreground">
                                    Informasi dasar unit, mesin, dan tanggal pengujian arus.
                                </p>
                            </div>
                        ...
                        {/* Card 2: Penandatangan Dokumen (3 Kolom Sesuai Scan) */}
                        <div className="rounded-lg border border-border bg-card p-4 space-y-4">
                            <div className="border-b border-border pb-2">
                                <h3 className="text-sm font-semibold text-foreground">
                            ...
                            <div className="space-y-4 text-xs">
                                ...
                                <div className="space-y-2 rounded-md border border-border/70 p-2.5 bg-muted/20">
                                    ...
                                    >
                                        <SelectTrigger className="h-7 text-xs">
                                            <SelectValue placeholder="Pilih Team Leader Pemeliharaan..." />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {tl_options.map((e) => (
                                                <SelectItem key={e.id} value={String(e.id)}>
                        ...
                        <div className="flex flex-1 flex-col justify-between rounded-lg border border-border bg-card shadow-sm">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4">
                                <div>
                                    ...
                                    <p className="text-[12px] text-muted-foreground">
                                        Kop surat, spesifikasi mesin, tabel arus motor, catatan, dan tanda tangan otomatis disesuaikan.
                                    </p>
                                </div>
                            ...
                            {/* TAB 1: FORM MATRIX INPUT */}
                            {viewTab === 'form' && (
                                <div className="p-4 space-y-6">
                                    {/* Action buttons on top of table */}
                                    <div className="flex flex-wrap items-center justify-between gap-3 bg-muted/40 p-3 rounded-lg border border-border">
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs font-semibold text-foreground">
                                                Daftar Elektro-Motor ({items.length} Unit)
                                            </span>
                                        </div>
                                    ...
                                    <div className="overflow-x-auto rounded-lg border border-border">
                                        <table className="w-full text-xs text-left">
                                            <thead className="bg-muted/80 text-muted-foreground uppercase text-[10px] tracking-wider border-b border-border">
                                                <tr>
                                                    <th rowSpan={2} className="p-2.5 text-center w-12 border-r border-border">
                                                        NO
                                                    </th>
                                                    <th rowSpan={2} className="p-2.5 min-w-[200px] border-r border-border">
                                                        NAMA ELEKTRO-MOTOR
                                                    </th>
                                                    <th colSpan={3} className="p-2 text-center border-r border-border">
                                                        ARUS PER PHASA (AMPERE)
                                                    </th>
                                                    <th rowSpan={2} className="p-2.5 min-w-[140px] border-r border-border">
                                                        KETERANGAN
                                                    </th>
                                                    <th rowSpan={2} className="p-2 text-center w-12">
                                                        AKSI
                                                    </th>
                                                </tr>
                                                <tr>
                                                    <th className="p-1.5 text-center w-20 border-t border-r border-border">R</th>
                                                    <th className="p-1.5 text-center w-20 border-t border-r border-border">S</th>
                                                    <th className="p-1.5 text-center w-20 border-t border-r border-border">T</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-border">
                                                {items.map((item, idx) => (
                                                    <tr key={idx} className="hover:bg-muted/30 transition-colors">
                                                        <td className="p-2 text-center font-bold border-r border-border text-muted-foreground">
                                                            {item.no}
                                                        </td>
                                                        <td className="p-2 border-r border-border">
                                                            <Input
                                                                value={item.motor_name}
                                                                onChange={(e) => updateItemField(idx, 'motor_name', e.target.value)}
                                                                placeholder="Nama elektro motor"
                                                                className="h-8 text-xs"
                                                            />
                                                        </td>
                                                        <td className="p-2 border-r border-border">
                                                            <Input
                                                                value={item.current_r}
                                                                onChange={(e) => updateItemField(idx, 'current_r', e.target.value)}
                                                                placeholder="17.4"
                                                        ...
                                                        <td className="p-2 border-r border-border">
                                                            <Input
                                                                value={item.current_s}
                                                                onChange={(e) => updateItemField(idx, 'current_s', e.target.value)}
                                                                placeholder="17.6"
                                                                className="h-8 text-xs text-center font-semibold"
                                                            />
                                                        </td>
                                                        <td className="p-2 border-r border-border">
                                                            <Input
                                                                value={item.current_t}
                                                                onChange={(e) => updateItemField(idx, 'current_t', e.target.value)}
                                                                placeholder="17.5"
                                                                className="h-8 text-xs text-center font-semibold"
                                                            />
                                                        </td>
                                    ...
                                    <div className="space-y-2 rounded-lg border border-border bg-card p-4">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-[11px] text-muted-foreground">
                                                Muncul di kotak catatan formulir PDF
                                            </span>
                                        </div>
                                        <textarea
                                            value={generalNotes}
                                            onChange={(e) => setGeneralNotes(e.target.value)}
                                            placeholder="Tuliskan catatan teknis atau kesimpulan hasil pengukuran arus motor..."
                                            rows={3}
                            ...
                            {viewTab === 'html' && (
                                <div className="p-4 space-y-3">
                                    <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-300">
                                        ...
                                        <strong>Mode Editor Teks Bebas:</strong> Anda dapat mengedit langsung kode HTML layout formulir ini. Saat disimpan dengan mode ini, sistem akan mencetak dokumen sesuai tampilan editor teks berikut.
                                    </div>
                                    <RichTextEditor
                                        value={htmlContent}
                    ...
                    <DialogContent className="max-w-2xl">
                        <DialogHeader>
                            <DialogTitle className="flex items-center gap-2">
                                ...
                                Riwayat Data Pengukuran Arus Kerja Elektro Motor
                            </DialogTitle>
                            <DialogDescription>
                                Daftar arsip pengujian untuk unit {unit.name} yang tersimpan di sistem.
                            </DialogDescription>
                        </DialogHeader>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        if (confirm('Apakah Anda yakin ingin menghapus arsip formulir ini?')) {
                                                            router.delete(
                                                                motorCurrentRoutes.destroy({ motorCurrent: item.id }).url,
                                                                {
                                                                    preserveScroll: true,
