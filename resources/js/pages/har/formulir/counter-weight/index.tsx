...
type Props = {
    ...
    form_data: {
        ...
        notes: string;
        measurements: CounterWeightMeasurement[];
...
export default function CounterWeightIndex({
    unit,
    units,
    machines,
    selected_machine_id,
    ...
    const [docNumber, setDocNumber] = useState<string>(
        form_data.document_number || 'FMKD-314-10.3.3.a-B8'
    );
    ...
    // Standards & General Notes
    const [standardAllowed, setStandardAllowed] = useState<string>(
        form_data.standard_allowed || 'Torsi Pengencangan Sesuai Manual Book / Kondisi Baik'
    );
    const [generalNotes, setGeneralNotes] = useState<string>(
        form_data.notes || 'Kondisi fisik baut counter weight terikat kencang, lock wire/locking washer terpasang sempurna, dan tidak ditemukan keretakan ataupun kelonggaran.'
    );
    ...
    // Update single cylinder condition / notes
    const updateCylinderField = (
        cylinder: number,
        field: keyof Omit<CounterWeightMeasurement, 'cylinder'>,
        val: any
    ) => {
        setMeasurements((prev) =>
            prev.map((item) =>
    ...
    const handleFillScanExample = () => {
        ...
        setStandardAllowed('Torsi Pengencangan Sesuai Manual Book / Kondisi Baik');
        setGeneralNotes('Kondisi fisik baut counter weight terikat kencang, lock wire/locking washer terpasang sempurna, dan tidak ditemukan keretakan ataupun kelonggaran.');
    };

    // When unit changes in selector
    const handleUnitChange = (newUnitId: string) => {
        router.get(
            counterWeightRoutes.index().url,
...
setInstalledPower(m.capacity_kw);
    ...
    const previewPdfUrl = useMemo(() => {
        ...
        measurements.forEach((m, idx) => {
            ...
            params.set(`measurements[${idx}][condition]`, m.condition);
    ...
    const handleSave = (onSuccessCallback?: () => void) => {
        ...
        router.post(counterWeightRoutes.store().url, payload, {
            preserveScroll: true,
            onSuccess: () => {
                setPreviewKey((k) => k + 1);
    ...
    return (
        <>
            <Head title={`Formulir Pemeriksaan Kondisi Kekencangan Baut Counter Weight - ${unit.name}`} />
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Select value={lineSpacing} onValueChange={setLineSpacing}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="1.1">1,1 (Sedang)</SelectItem>
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border bg-muted/20 p-2.5 text-xs">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <Button
                                                ...
                                                className="h-7 gap-1 text-[11px] text-emerald-700 dark:text-emerald-400 border-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-950/30"
                                            >
...
/>
                                        ...
                                        <table className="w-full border-collapse text-xs min-w-[700px]">
                                            <thead>
                                                <tr className="border-b border-border bg-muted/40 font-semibold text-center">
                                                    <th className="border-r border-border p-2 w-16">CYL NO.</th>
                                                    <th className="border-r border-border p-2 w-12">NO</th>
                                                    <th className="border-r border-border p-2 w-64">KONDISI BAUT COUNTER WEIGHT</th>
                                            ...
                                            <tbody className="divide-y divide-border">
                                                {measurements.map((row) => (
                                                    <tr key={row.cylinder} className="hover:bg-muted/20 transition-colors">
                                                        ...
                                                        <td className="border-r border-border p-2 text-center text-muted-foreground font-mono">
                                                            1 &amp; 2
                                                        </td>
                                                        <td className="border-r border-border p-2">
                                                            <div className="flex items-center gap-2">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => updateCylinderField(row.cylinder, 'condition', 'baik')}
                                                                    style={
                                                                        row.condition === 'baik'
                                                                ...
                                                                >
                                                                    <span
                                                                        ...
                                                                        style={{
                                                                            backgroundColor: row.condition === 'baik' ? '#ffffff' : 'transparent',
                                                                            borderColor: row.condition === 'baik' ? '#ffffff' : 'currentColor',
                                                                        }}
                                                                    />
                                                                    1. Baik
                                                                </button>
                                                                <button
                                                                    type="button"
                                                                    onClick={() => updateCylinderField(row.cylinder, 'condition', 'tidak_baik')}
                                                                    style={
                                                                        row.condition === 'tidak_baik'
                                                                ...
                                                                >
                                                                    <span
                                                                        className="inline-block size-3 rounded-xs border"
                                                                        style={{
                                                                            backgroundColor: row.condition === 'tidak_baik' ? '#ffffff' : 'transparent',
                                                                            borderColor: row.condition === 'tidak_baik' ? '#ffffff' : 'currentColor',
                                                                        }}
                                                                    />
                                                                    2. Tidak Baik
                                                                </button>
                                                        ...
                                                        <td className="p-2">
                                                            <Input
                                                                value={row.notes}
                                                                onChange={(e) =>
                                                                    updateCylinderField(row.cylinder, 'notes', e.target.value)
                                                                }
                                                                placeholder="Catatan baut silinder..."
                                                                className="h-7 text-xs"
                                                            />
                                                        </td>
                                                    </tr>
                                    ...
                                    <div className="grid gap-3 md:grid-cols-2">
                                        <div className="rounded-md border border-border bg-muted/20 p-3 space-y-1.5 text-xs">
                                            ...
                                            <Input
                                                ...
                                                placeholder="Torsi Pengencangan Sesuai Manual Book / Kondisi Baik"
                                                className="h-8 text-xs bg-background"
                                            />
                                            <p className="text-[11px] text-muted-foreground">
                                                Standar toleransi atau pedoman kekencangan torsi baut counter weight.
                                            </p>
                                        </div>

                                        <div className="space-y-1.5 text-xs">
                                            <Label className="font-semibold">Catatan Umum / Hasil Pemeriksaan :</Label>
                                            <textarea
                                                rows={3}
                                                value={generalNotes}
                                                onChange={(e) => setGeneralNotes(e.target.value)}
                                                placeholder="Tuliskan catatan kondisi pengencangan baut, lock wire, locking washer..."
                            ...
                            {viewTab === 'pdf' && (
                                <div className="p-4 space-y-3">
                                    ...
                                    <PdfPreviewFrame
                                        ...
                                        title="Pratinjau PDF Baut Counter Weight"
                                        src={previewPdfUrl}
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (confirm('Yakin ingin menghapus formulir yang tersimpan ini?')) {
                                                    router.delete(
                                                        counterWeightRoutes.destroy({ counterWeight: record.id }).url
                                                    );
                                                }
                                            }}
                                            className="gap-1.5 text-xs"
            ...
            <Dialog open={showHistory} onOpenChange={setShowHistory}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Riwayat Formulir Baut Counter Weight - {unit.name}</DialogTitle>
                        <DialogDescription>
                            Daftar catatan pemeriksaan kondisi kekencangan baut counter weight yang pernah disimpan untuk unit dan mesin ini.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="max-h-[380px] overflow-y-auto divide-y divide-border">
                        {history.length === 0 ? (
                            <div className="py-8 text-center text-xs text-muted-foreground">
                                Belum ada riwayat pemeriksaan untuk mesin ini.
                            </div>
                        ) : (
                            history.map((item) => (
                                <div
