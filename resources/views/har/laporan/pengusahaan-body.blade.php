@php
    ...
    $rawUnitName = $report['unit']['name'] ?? 'PLTD Poasia Containerized';
    $unitHeaderName = strtoupper($rawUnitName);
    if (!str_contains($unitHeaderName, 'SITE') && str_contains($unitHeaderName, 'POASIA')) {
        $unitHeaderName .= ' 6 SITE';
    }
    $unitDisplayName = ucwords(strtolower($rawUnitName));
    $pengesahanDate = 'Kendari, 1 ' . \App\Support\Indonesian::monthName($periodMonth) . ' ' . $periodYear;

    $signatories = $data['document']['signatories'] ?? ($report['signatories'] ?? []);
    $tlHarName = $signatories['tl_har']['name'] ?? '';
    $tlHarPosition = $signatories['tl_har']['position'] ?? 'Team Leader Pemeliharaan';
    $tlHarSignature = $signatories['tl_har']['signature'] ?? null;

    $koordinatorName = $signatories['staff_har']['name'] ?? ($signatories['koordinator']['name'] ?? '');
    $koordinatorPosition = $signatories['staff_har']['position'] ?? ($signatories['koordinator']['position'] ?? 'Koordinator Pemeliharaan');
    $koordinatorSignature = $signatories['staff_har']['signature'] ?? ($signatories['koordinator']['signature'] ?? null);

    $managerName = $signatories['manager_ul']['name'] ?? ($signatories['manager']['name'] ?? '');
    $managerPosition = $signatories['manager_ul']['position'] ?? ($signatories['manager']['position'] ?? 'Manager UL');
    $managerSignature = $signatories['manager_ul']['signature'] ?? ($signatories['manager']['signature'] ?? null);

    $projectLeaderName = $signatories['project_leader']['name'] ?? '';
    $projectLeaderPosition = $signatories['project_leader']['position'] ?? 'Project Leader';
    $projectLeaderSignature = $signatories['project_leader']['signature'] ?? null;

    $koordinatorHarName = $signatories['koordinator_har']['name'] ?? ($signatories['staff_har']['name'] ?? '');
    $koordinatorHarPosition = $signatories['koordinator_har']['position'] ?? 'Koordinator Pemeliharaan';
    $koordinatorHarSignature = $signatories['koordinator_har']['signature'] ?? ($signatories['staff_har']['signature'] ?? null);

    $resumeData = $report['resume_statistik'] ?? null;
    $resumeRows = $resumeData['rows'] ?? [
    ...
    $sections = [
        ...
        'Formulir Pemeriksaan Axial Conrod & Baut Conrod',
        ...
        'Formulir Pengukuran Tegangan Baterai',
...
{{-- 5. ISTILAH DAN DEFINISI (Sesuai Format Standar PLN NP) --}}
<div class="break-before" id="sec-5">
    <table style="width:100%; border-collapse:collapse; border:1px solid #000; font-family:'DejaVu Sans', Arial, sans-serif; margin-bottom:12px;">
        <tr>
            ...
            <td style="width:90px; text-align:center; vertical-align:middle; padding:4px 6px; border-left:1px solid #000;">
                <img src="/logo/k3.png" alt="K3" style="height:42px;">
            </td>
        </tr>
    </table>
...
{{-- 6. SERVICE REQUEST MAP (Sesuai Format Standar PLN NP) --}}
...
{{-- 7. SERVICE REQUEST SUMMARY (Sesuai Format Standar PLN NP) --}}
...
<div class="break-before" id="sec-8">
    ...
    {{-- 3. Chart: Maintenance Mix Bulan ini (3D / Perspective Pie Chart SVG) --}}
    @php
        $pmPctVal = 93.8;
        $cmPctVal = 6.2;
        foreach ($tasksRows as $tr) {
...
{{-- 9. ISI LAPORAN --}}
<div class="har-h2 break-before" id="sec-9">9. Isi Laporan</div>
<p class="har-p">
...
<div class="har-h3">10.1 Rencana vs Realisasi{!! $num('schedules') !!}</div>
...
<div class="har-h3">10.2 Log Kegiatan HARMES{!! $num('activities') !!}</div>
...
{{-- 10. WORK ORDER SUMMARY (FIX) --}}
<div class="har-h2 break-before" id="sec-10">10. Work Order Summary (Fix){!! $num('wo_summary') !!}</div>
<table class="har-data">
...
{{-- 13. WO PM (WO PREVENTIVE MAINTANANCE - FMKD-314-10.3.3-A12) --}}
...
{{-- 14. WO PDM (WO PREDICTIVE MAINTANANCE - FMKD-314-10.3.3-A13) --}}
...
{{-- 15. WO CM (WO CORRECTIVE MAINTANANCE - FMKD-314-10.3.3-A14) --}}
<div class="break-before" id="sec-15">
    <table style="width:100%; border-collapse:collapse; border:1px solid #000; font-family:'DejaVu Sans', Arial, sans-serif; margin-bottom:10px;">
        <tr>
            <td style="width:170px; text-align:left; vertical-align:middle; padding:6px 8px; border:1px solid #000; border-bottom:none;">
                <img src="/logo/sidebar-logo.png" alt="PLN Nusantara Power" style="height:34px;">
...
@php
    $formulirList = $report['formulir'] ?? [];
    $secIndex = 19;
    $formNumber = 18;
@endphp

@foreach($formulirList as $fIdx => $form)
    @php
        $currentSecId = 'sec-' . ($secIndex + $fIdx);
        $currentFormNum = $formNumber + $fIdx;
    @endphp

    @foreach($form['sheets'] as $sIdx => $sheet)
        <div class="har-formulir-page" @if($sIdx === 0) id="{{ $currentSecId }}" @endif>
            <div class="har-h2" style="margin: 0 0 4px 0; font-size: 10px; color: #0b2545;">
                {{ $currentFormNum }}. {{ $form['title'] }} — {{ $sheet['machine_name'] }}
            </div>
            <div class="har-formulir-wrapper">
                {!! $sheet['html'] !!}
            </div>
        </div>
    @endforeach
...
{{-- LAMPIRAN FOTO KEGIATAN --}}
@php
    $lampiranSecId = 'sec-' . ($secIndex + count($formulirList));
    $lampiranNum = $formNumber + count($formulirList);
@endphp
<div class="har-h2 break-before" id="{{ $lampiranSecId }}">{{ $lampiranNum }}. Lampiran Foto Kegiatan</div>
