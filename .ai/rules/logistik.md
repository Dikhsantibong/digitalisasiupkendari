...
## Logistik: satu folder halaman per form & per sheet
FormController merender `logistik/input/{form}/index`, JadwalSheetController merender `logistik/{menu}/{sheet}/index` (menu dari LogistikJadwal::SHEETS: jadwal|input). Tambah form/sheet baru WAJIB juga membuat halaman tipis di folder itu yang merender `LogistikFormInputPage` (components/logistik/form-input-page.tsx) atau `LogistikJadwalSheetPage` (components/logistik/jadwal-sheet-page.tsx) + breadcrumb helper. URL/route tidak berubah. inertia.testing.ensure_pages_exist=true → tes gagal bila folder lupa dibuat. Tipe kolom form: date & check (+exclusive = satu centang per baris per grup).
