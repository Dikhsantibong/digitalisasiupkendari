...
class HarInjectorPressurePdfBuilder
{
    /**
     * Build view data for the Injector Pressure PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($measurements) || count($measurements) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $measurements[] = [
                    'cylinder' => $i,
                    'pressure_before' => $existingMap[$i]['pressure_before'] ?? '',
                    'pressure_after' => $existingMap[$i]['pressure_after'] ?? '',
                    'nozzle_holes' => $existingMap[$i]['nozzle_holes'] ?? '',
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B1',
            'revision' => $input['revision'] ?? '02',
            ...
            'serial_number' => $input['serial_number'] ?? ($machine?->serial_number ?? '26881'),
            'machine_number' => $input['machine_number'] ?? ($machine ? str_replace(['MIRRLEES #', 'MESIN #', 'UNIT #'], '', $machine->name) : '1'),
            ...
            'standard_allowed' => $input['standard_allowed'] ?? '270 kg/cm²',
            'visual_inspection' => $input['visual_inspection'] ?? 'Kondisi fisik injektor bersih, semprotan mengabut sempurna, tidak ada tetesan (dribbling).',
            ...
            // Logos and Illustrations Base64
            'logo_pln' => $this->fileToBase64(public_path('logo/sidebar-logo.png')),
            'logo_k3' => $this->fileToBase64(public_path('logo/k3.png')),
            'injector_diagram' => $this->fileToBase64(public_path('images/formulir/injector_diagram.png')),
        ];
    }
    ...
    {
        return view('har.formulir.injector-pressure-pdf', ['data' => $data])->render();
    }
