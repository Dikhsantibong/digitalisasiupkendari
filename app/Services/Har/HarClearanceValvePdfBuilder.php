...
class HarClearanceValvePdfBuilder
{
    /**
     * Build view data for the Clearance Valve PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($measurements) || count($measurements) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $measurements[] = [
                    'cylinder' => $i,
                    'ex_before_r' => $existingMap[$i]['ex_before_r'] ?? '',
                    'ex_before_l' => $existingMap[$i]['ex_before_l'] ?? '',
                    'ex_after_r' => $existingMap[$i]['ex_after_r'] ?? '',
                    'ex_after_l' => $existingMap[$i]['ex_after_l'] ?? '',
                    'in_before_r' => $existingMap[$i]['in_before_r'] ?? '',
                    'in_before_l' => $existingMap[$i]['in_before_l'] ?? '',
                    'in_after_r' => $existingMap[$i]['in_after_r'] ?? '',
                    'in_after_l' => $existingMap[$i]['in_after_l'] ?? '',
                ];
            }
        }
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B3',
            ...
            'standard_ex' => $input['standard_ex'] ?? '0.60 mm',
            'standard_in' => $input['standard_in'] ?? '0.30 mm',
            'standard_allowed' => $input['standard_allowed'] ?? null,
    ...
    {
        return view('har.formulir.clearance-valve-pdf', ['data' => $data])->render();
    }
