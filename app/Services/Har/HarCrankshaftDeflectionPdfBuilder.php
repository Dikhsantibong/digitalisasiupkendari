...
class HarCrankshaftDeflectionPdfBuilder
{
    /**
     * Build view data for the Crankshaft Deflection PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($measurements) || count($measurements) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $measurements[] = [
                    'cylinder' => $i,
                    'pos_a' => $existingMap[$i]['pos_a'] ?? '0',
                    'pos_b' => $existingMap[$i]['pos_b'] ?? '0',
                    'pos_c' => $existingMap[$i]['pos_c'] ?? '0',
                    'pos_d' => $existingMap[$i]['pos_d'] ?? '0',
                    'pos_e' => $existingMap[$i]['pos_e'] ?? '0',
                ];
            }
        }
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B2',
            ...
            'standard_min' => $input['standard_min'] ?? '-0.06',
            'standard_max' => $input['standard_max'] ?? '+0.08',
            'standard_allowed' => $input['standard_allowed'] ?? 'Min : -0.06, Max : +0.08',
            ...
            'diagram_image' => $this->fileToBase64(public_path('images/har/crankshaft-deflection-diagram.png')),
        ];
    }
    ...
    {
        return view('har.formulir.crankshaft-deflection-pdf', ['data' => $data])->render();
    }
