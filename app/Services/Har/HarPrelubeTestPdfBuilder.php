...
class HarPrelubeTestPdfBuilder
{
    /**
     * Build view data for the Prelube Test PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($items) || count($items) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $items[] = [
                    'cylinder' => $i,
                    'camshaft' => $existingMap[$i]['camshaft'] ?? 'v',
                    'conrod' => $existingMap[$i]['conrod'] ?? 'v',
                    'piston' => $existingMap[$i]['piston'] ?? 'v',
                    'rocker_arm' => $existingMap[$i]['rocker_arm'] ?? 'v',
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B12',
            ...
            'machine_number' => $input['machine_number'] ?? ($machine?->name ?? '1'),
    ...
    {
        return view('har.formulir.prelube-test-pdf', ['data' => $data])->render();
    }
