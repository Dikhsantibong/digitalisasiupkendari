...
class HarCounterWeightPdfBuilder
{
    /**
     * Build view data for the Counter Weight PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($measurements) || count($measurements) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $measurements[] = [
                    'cylinder' => $i,
                    'condition' => $existingMap[$i]['condition'] ?? 'baik',
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B8',
            ...
            'standard_allowed' => $input['standard_allowed'] ?? 'Torsi Pengencangan Sesuai Manual Book / Kondisi Baik',
            'notes' => $input['notes'] ?? 'Kondisi fisik baut counter weight terikat kencang, lock wire/locking washer terpasang sempurna, dan tidak ditemukan keretakan ataupun kelonggaran.',
    ...
    {
        return view('har.formulir.counter-weight-pdf', ['data' => $data])->render();
    }
