...
class HarHydrotestPdfBuilder
{
    /**
     * Build view data for the Hydrotest PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        if (empty($items) || count($items) !== $cylindersCount) {
            ...
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $items[] = [
                    'cylinder' => $i,
                    'oring_liner' => $existingMap[$i]['oring_liner'] ?? 'v',
                    'liner' => $existingMap[$i]['liner'] ?? 'v',
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B11',
            ...
            'machine_number' => $input['machine_number'] ?? ($machine?->name ?? '4'),
            ...
            'standard_allowed' => $input['standard_allowed'] ?? 'Tidak ada kebocoran',
    ...
    {
        return view('har.formulir.hydrotest-pdf', ['data' => $data])->render();
    }
