...
class HarTimingInjectionPumpPdfBuilder
{
    /**
     * Build view data for the Timing Injection Pump PDF and HTML document.
     *
    ...
    public function buildData(Unit $unit, ?Machine $machine = null, array $input = []): array
    {
        ...
        // Pre-fill or use provided checklist items
        $items = $input['checklist_items'] ?? [];
        if (empty($items) || count($items) !== $cylindersCount) {
            $existingMap = collect($items)->keyBy('cylinder')->all();
            $items = [];
            for ($i = 1; $i <= $cylindersCount; $i++) {
                $items[] = [
                    'cylinder' => $i,
                    'timing_before' => $existingMap[$i]['timing_before'] ?? 'v',
                    'timing_after' => $existingMap[$i]['timing_after'] ?? 'v',
        ...
        return [
            ...
            'machine' => $machine,
            'ul_label' => $ulLabel,
            'document_number' => $input['document_number'] ?? 'FMKD-314-10.3.3.a-B5',
            ...
            'machine_number' => $input['machine_number'] ?? ($machine ? str_replace('MIRRLEES #', '', $machine->name) : '4'),
    ...
    {
        return view('har.formulir.timing-injection-pump-pdf', ['data' => $data])->render();
    }
