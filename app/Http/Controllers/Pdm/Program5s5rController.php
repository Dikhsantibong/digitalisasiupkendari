...
class Program5s5rController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $days = collect(range(1, $daysInMonth))->map(function (int $day) use ($year, $month, $holidays): array {
            ...
            // Singkatan 2 huruf standar Indonesia (JM, SB, MG, SN, SL, RB, KM)
        ...
        if (! $meta->exists) {
            ...
            $meta->doc_number = $meta->doc_number ?: 'PLN-NP-UPKDR/JADWAL-5S5R-PDM';
        ...
        $savedRecords = PdmJadwal5s5r::query()
            ->where('unit_id', $unit->id)
            ->where('year', $year)
            ->where('month', $month)
            ->orderBy('sort_order')
        ...
        if ($savedRecords->isEmpty()) {
            ...
            $rows = $savedRecords->map(fn (PdmJadwal5s5r $r, int $idx): array => [
                'id' => $r->id,
        ...
        return Inertia::render('pdm/jadwal/program-5s-5r/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Jadwal Pelaksanaan 5S5R PdM {$unit->name} {$month}/{$year}",
            $unit,
            unit: $unit->id,
        );
        ...
        return back()->with('toast', [
            'type' => 'success',
            'message' => 'Jadwal Pelaksanaan 5S5R PdM Pembangkit berhasil disimpan.',
        ]);
    }
    ...
    public function pdf(Request $request): HttpResponse
    {
        ...
        $filename = sprintf(
            'Jadwal_Pelaksanaan_5S5R_PdM_%s_%02d_%d.pdf',
    ...
    public function pdfView(Unit $unit, int $month, int $year): array
    {
        ...
        return ['pdm.jadwal.program-5s-5r-pdf', [
            'unit' => $unit,
            'year' => $year,
            'month' => $month,
