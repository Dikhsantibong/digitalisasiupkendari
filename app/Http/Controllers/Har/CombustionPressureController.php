...
class CombustionPressureController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        if ($request->filled('record_id')) {
            $record = HarCombustionPressure::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarCombustionPressure::query()
                ->where('unit_id', $unit->id)
        ...
        $history = HarCombustionPressure::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/combustion-pressure/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.combustion-pressure.pdf', [
                'unit_id' => $unit->id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $record = HarCombustionPressure::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Pengukuran Tekanan Pembakaran {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Pengukuran Tekanan Pembakaran berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarCombustionPressure::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarCombustionPressure::query()
                ->where('unit_id', $unit->id)
        ...
        $filename = "Tekanan-Pembakaran-{$unit->name}-{$safeDate}.pdf";
    ...
    public function destroy(HarCombustionPressure $combustionPressure): RedirectResponse
    {
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Pengukuran Tekanan Pembakaran berhasil dihapus.',
        ]);
