...
class BatteryVoltageController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        if ($request->filled('record_id')) {
            $record = HarBatteryVoltage::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarBatteryVoltage::query()
                ->where('unit_id', $unit->id)
        ...
        $inputData = $record ? $record->toArray() : [
            ...
            'machine_number' => $machine ? str_replace(['MIRRLEES #', 'MESIN #', 'UNIT #'], '', $machine->name) : '1,2,3',
        ...
        $history = HarBatteryVoltage::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/battery-voltage/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.battery-voltage.pdf', [
                'unit_id' => $unit->id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'cells_24v' => ['required', 'array'],
            'summary_24v' => ['nullable', 'array'],
            'cells_110v' => ['required', 'array'],
            'summary_110v' => ['nullable', 'array'],
        ...
        // Auto compute summaries if empty
        if (empty($validated['summary_24v']['total'])) {
            $validated['summary_24v'] = HarBatteryVoltagePdfBuilder::calculateSummary($validated['cells_24v']);
        }
        if (empty($validated['summary_110v']['total'])) {
            $validated['summary_110v'] = HarBatteryVoltagePdfBuilder::calculateSummary($validated['cells_110v']);
        }

        $record = HarBatteryVoltage::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Pengukuran Tegangan Battery {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Pengukuran Tegangan Battery berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarBatteryVoltage::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarBatteryVoltage::query()
                ->where('unit_id', $unit->id)
        ...
        if ($record && $record->format === 'html' && ! empty($record->content_html) && ! $request->has('cells_24v')) {
            ...
            $data = $record ? $record->toArray() : [
                ...
                'machine_number' => $request->input('machine_number', $machine ? str_replace(['MIRRLEES #', 'MESIN #', 'UNIT #'], '', $machine->name) : '1,2,3'),
                ...
                'cells_24v' => $request->input('cells_24v', HarBatteryVoltagePdfBuilder::defaultCells24v()),
                'summary_24v' => $request->input('summary_24v'),
                'cells_110v' => $request->input('cells_110v', HarBatteryVoltagePdfBuilder::defaultCells110v()),
                'summary_110v' => $request->input('summary_110v'),
                'charging_conditions' => $request->input('charging_conditions', HarBatteryVoltagePdfBuilder::DEFAULT_CHARGING_CONDITIONS),
                ...
                'page_margin_top' => $request->integer('page_margin_top', 8),
                'page_margin_bottom' => $request->integer('page_margin_bottom', 8),
                'page_margin_left' => $request->integer('page_margin_left', 10),
                'page_margin_right' => $request->integer('page_margin_right', 10),
                'line_spacing' => $request->input('line_spacing', '1.1'),
            ];
        ...
        $filename = sprintf(
            'Formulir-Pengukuran-Tegangan-Battery-%s-%s-%s.pdf',
    ...
    public function destroy(Request $request, HarBatteryVoltage $record): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            "Menghapus Formulir Pengukuran Tegangan Battery {$unitName} tanggal {$date}",
            unit: $record->unit_id,
        );
