...
class InjectorPressureController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        if ($request->filled('record_id')) {
            $record = HarInjectorPressure::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarInjectorPressure::query()
                ->where('unit_id', $unit->id)
        ...
        $inputData = $record ? $record->toArray() : [
            'test_date' => $testDate,
            'document_number' => 'FMKD-314-10.3.3.a-B1',
            'revision' => '02',
            'effective_date' => '31 Juli 2024',
            'brand' => 'MAK',
            'model_type' => $machine?->type ?? '8M 453 AK',
            'serial_number' => $machine?->serial_number ?? '26881',
            ...
            'visual_inspection' => 'Kondisi fisik injektor bersih, semprotan mengabut sempurna, tidak ada tetesan (dribbling).',
        ];
        ...
        $history = HarInjectorPressure::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/injector-pressure/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.injector-pressure.pdf', [
                'unit_id' => $unit->id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $record = HarInjectorPressure::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Pengukuran Tekanan Pengabutan Injektor {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Pengukuran Tekanan Pengabutan Injektor berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarInjectorPressure::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarInjectorPressure::query()
                ->where('unit_id', $unit->id)
        ...
        if ($record && $record->format === 'html' && ! empty($record->content_html) && ! $request->has('cylinders_count')) {
            ...
            $data = $record ? $record->toArray() : [
                'test_date' => $testDate,
                'document_number' => $request->input('document_number', 'FMKD-314-10.3.3.a-B1'),
                ...
                'serial_number' => $request->input('serial_number', $machine?->serial_number ?? '26881'),
                ...
                'standard_allowed' => $request->input('standard_allowed', '270 kg/cm²'),
                'visual_inspection' => $request->input('visual_inspection', 'Kondisi fisik injektor bersih, semprotan mengabut sempurna, tidak ada tetesan (dribbling).'),
        ...
        $filename = sprintf(
            'Formulir-Tekanan-Pengabutan-Injektor-%s-%s-%s.pdf',
    ...
    public function destroy(Request $request, HarInjectorPressure $record): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            "Menghapus Formulir Pengukuran Tekanan Pengabutan Injektor {$unitName} tanggal {$date}",
            unit: $record->unit_id,
        );
