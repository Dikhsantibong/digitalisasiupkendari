...
class PrelubeTestController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        if ($request->filled('record_id')) {
            $record = HarPrelubeTest::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarPrelubeTest::query()
                ->where('unit_id', $unit->id)
        ...
        // History of past tests for this unit/machine
        $history = HarPrelubeTest::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/prelube-test/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.prelube-test.pdf', [
                'unit_id' => $unit->id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $record = HarPrelubeTest::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Checklist Prelube Test {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Checklist Prelube Test berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarPrelubeTest::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarPrelubeTest::query()
                ->where('unit_id', $unit->id)
        ...
        $filename = "Prelube-Test-{$unit->name}-{$safeDate}.pdf";
    ...
    public function destroy(HarPrelubeTest $prelubeTest): RedirectResponse
    {
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Prelube Test berhasil dihapus.',
        ]);
