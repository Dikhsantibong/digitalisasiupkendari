...
class CounterWeightController extends Controller
{
    use AuthorizesFieldInput;

    public function __construct(
        private readonly HarCounterWeightPdfBuilder $pdfBuilder,
    ...
    public function index(Request $request): InertiaResponse
    {
        $user = $request->user();
        abort_unless(
            $this->allowsFieldInput($user, PermissionName::HarInputView, PermissionName::HarLapanganCounterWeight) ||
        ...
        if ($request->filled('record_id')) {
            $record = HarCounterWeight::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarCounterWeight::query()
                ->where('unit_id', $unit->id)
        ...
        $inputData = $record ? $record->toArray() : [
            'test_date' => $testDate,
            'document_number' => 'FMKD-314-10.3.3.a-B8',
            'revision' => '02',
            ...
            'notes' => 'Kondisi fisik baut counter weight terikat kencang, lock wire/locking washer terpasang sempurna, dan tidak ditemukan keretakan ataupun kelonggaran.',
        ];
        ...
        $history = HarCounterWeight::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/counter-weight/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.counter-weight.pdf', [
                'unit_id' => $unit->id,
                'machine_id' => $machine?->id,
                'test_date' => $testDate,
                'record_id' => $record?->id,
            ]),
            'can_write' => $this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganCounterWeight),
        ]);
    }
    ...
    {
        $user = $request->user();
        abort_unless($this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganCounterWeight), 403);
        ...
        $record = HarCounterWeight::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Pemeriksaan Kondisi Kekencangan Baut Counter Weight {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Pemeriksaan Kondisi Kekencangan Baut Counter Weight berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarCounterWeight::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarCounterWeight::query()
                ->where('unit_id', $unit->id)
        ...
        if ($record && $record->format === 'html' && ! empty($record->content_html) && ! $request->has('cylinders_count')) {
            ...
            $data = $record ? $record->toArray() : [
                'test_date' => $testDate,
                'document_number' => $request->input('document_number', 'FMKD-314-10.3.3.a-B8'),
                ...
                'machine_number' => $request->input('machine_number', $machine ? str_replace(['MIRRLEES #', 'MESIN #', 'UNIT #'], '', $machine->name) : '1'),
                ...
                'standard_allowed' => $request->input('standard_allowed', 'Torsi Pengencangan Sesuai Manual Book / Kondisi Baik'),
                'notes' => $request->input('notes', 'Kondisi fisik baut counter weight terikat kencang, lock wire/locking washer terpasang sempurna, dan tidak ditemukan keretakan ataupun kelonggaran.'),
        ...
        $filename = sprintf(
            'Formulir-Baut-Counter-Weight-%s-%s-%s.pdf',
    ...
    public function destroy(Request $request, HarCounterWeight $record): RedirectResponse
    {
        $user = $request->user();
        abort_unless($this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganCounterWeight), 403);
        ...
        $this->activityLogger->log(
            ...
            "Menghapus Formulir Pemeriksaan Kondisi Kekencangan Baut Counter Weight {$unitName} tanggal {$date}",
            unit: $record->unit_id,
        );
