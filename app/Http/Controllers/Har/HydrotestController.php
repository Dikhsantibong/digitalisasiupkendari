...
class HydrotestController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        if ($request->filled('record_id')) {
            $record = HarHydrotest::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarHydrotest::query()
                ->where('unit_id', $unit->id)
        ...
        // History of past hydrotests for this unit/machine
        $history = HarHydrotest::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/hydrotest/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.hydrotest.pdf', [
                'unit_id' => $unit->id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $record = HarHydrotest::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Checklist Hydrotest {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Checklist Hydrotest berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarHydrotest::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarHydrotest::query()
                ->where('unit_id', $unit->id)
        ...
        $filename = "Hydrotest-{$unit->name}-{$safeDate}.pdf";
    ...
    public function destroy(HarHydrotest $hydrotest): RedirectResponse
    {
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Hydrotest berhasil dihapus.',
        ]);
