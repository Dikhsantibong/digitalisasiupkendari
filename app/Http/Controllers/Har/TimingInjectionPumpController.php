...
class TimingInjectionPumpController extends Controller
{
    use AuthorizesFieldInput;

    public function __construct(
        private readonly HarTimingInjectionPumpPdfBuilder $pdfBuilder,
    ...
    public function index(Request $request): InertiaResponse
    {
        $user = $request->user();
        abort_unless(
            $this->allowsFieldInput($user, PermissionName::HarInputView, PermissionName::HarLapanganTimingInjectionPump) ||
        ...
        if ($request->filled('record_id')) {
            $record = HarTimingInjectionPump::query()->where('unit_id', $unit->id)->find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarTimingInjectionPump::query()
                ->where('unit_id', $unit->id)
        ...
        $inputData = $record ? $record->toArray() : [
            ...
            'machine_number' => $machine ? str_replace('MIRRLEES #', '', $machine->name) : '4',
        ...
        $history = HarTimingInjectionPump::query()
            ->where('unit_id', $unit->id)
        ...
        return Inertia::render('har/formulir/timing-injection-pump/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ...
            'history' => $history,
            'pdf_url' => route('har.formulir.timing-injection-pump.pdf', [
                'unit_id' => $unit->id,
                'machine_id' => $machine?->id,
                'test_date' => $testDate,
                'record_id' => $record?->id,
            ]),
            'can_write' => $this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganTimingInjectionPump),
        ]);
    }
    ...
    {
        $user = $request->user();
        abort_unless($this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganTimingInjectionPump), 403);
        ...
        $record = HarTimingInjectionPump::query()->updateOrCreate(
            [
        ...
        $this->activityLogger->log(
            ...
            "Menyimpan Formulir Checklist Timing Injection Pump {$unit->name} tanggal {$validated['test_date']}",
            $record,
            unit: $unit->id,
        );
        ...
        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Checklist Timing Injection Pump berhasil disimpan.',
        ]);
    ...
    public function pdf(Request $request): Response
    {
        ...
        $record = null;
        if ($request->filled('record_id')) {
            $record = HarTimingInjectionPump::find($request->integer('record_id'));
        }
        if (! $record && $machine) {
            $record = HarTimingInjectionPump::query()
                ->where('unit_id', $unit->id)
        ...
        $filename = "Timing-Injection-Pump-{$unit->name}-{$safeDate}.pdf";
    ...
    public function destroy(HarTimingInjectionPump $timingInjectionPump): RedirectResponse
    {
        $user = request()->user();
        abort_unless($this->allowsFieldInput($user, PermissionName::HarInputWrite, PermissionName::HarLapanganTimingInjectionPump), 403);
        abort_unless($user->canAccessUnit($timingInjectionPump->unit_id), 403);

        $timingInjectionPump->delete();

        Inertia::flash('toast', [
            'type' => 'success',
            'message' => 'Formulir Timing Injection Pump berhasil dihapus.',
        ]);
