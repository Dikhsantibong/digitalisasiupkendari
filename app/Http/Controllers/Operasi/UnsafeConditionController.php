...
class UnsafeConditionController extends Controller
{
    ...
    {
        $user = $request->user();
        abort_unless(($this->allowsFieldInput($user, PermissionName::OperasiInputView, PermissionName::OperasiLapanganUnsafeCondition) || $user->hasPermissionTo(PermissionName::OperasiLaporanView)), 403);
        ...
        $canWrite = $this->allowsFieldInput($user, PermissionName::OperasiInputWrite, PermissionName::OperasiLapanganUnsafeCondition);

        return Inertia::render('operasi/input/unsafe-condition/index', [
            'unit' => [
                'id' => $unit->id,
                'name' => $unit->name,
            ],
    ...
    {
        $user = $request->user();
        abort_unless($this->allowsFieldInput($user, PermissionName::OperasiInputWrite, PermissionName::OperasiLapanganUnsafeCondition), 403);
        ...
        $this->activityLogger->log(
            ...
            "Menambah laporan unsafe action/condition {$unit->name} (Operasi)",
            $unit,
            unit: $unit->id,
        );
    ...
    public function update(Request $request, HarUnsafeCondition $unsafeCondition): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            "Memperbarui laporan unsafe action/condition #{$unsafeCondition->id} (Operasi)",
    ...
    public function destroy(Request $request, HarUnsafeCondition $unsafeCondition): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            "Menghapus laporan unsafe action/condition #{$unsafeCondition->id} (Operasi)",
            unit: $unsafeCondition->unit_id,
        );
