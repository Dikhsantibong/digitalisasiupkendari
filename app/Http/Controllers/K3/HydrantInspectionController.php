...
class HydrantInspectionController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('k3/input/hydrant', [
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'rows.*.jenis' => ['nullable', 'string', 'max:255'],
            'rows.*.tanggal' => ['nullable', 'date'],
            'rows.*.hose' => ['nullable', 'string', 'max:255'],
            'rows.*.nozzle' => ['nullable', 'string', 'max:255'],
            'rows.*.box' => ['nullable', 'string', 'max:255'],
            'rows.*.tekanan' => ['nullable', 'string', 'max:255'],
        ...
        DB::transaction(function () use ($validated, $unit, $month, $year, $user): void {
            ...
            foreach ($validated['rows'] ?? [] as $index => $row) {
                ...
                if (! empty($row['id'])) {
                    $record = K3HydrantInspection::query()->where('id', $row['id'])->where('unit_id', $unit->id)->first();
                    if ($record) {
                ...
                $existingIds[] = K3HydrantInspection::create([...$attributes, 'unit_id' => $unit->id, 'year' => $year, 'month' => $month])->id;
            }

            K3HydrantInspection::query()
                ->where('unit_id', $unit->id)->where('year', $year)->where('month', $month)
                ->when($existingIds !== [], fn ($q) => $q->whereNotIn('id', $existingIds))
                ->delete();
        });

        $this->activityLogger->log(ActivityEvent::Updated, "Menyimpan Inspeksi Hydrant {$unit->name} {$month}/{$year}", $unit, unit: $unit->id);

        return back()->with('toast', ['type' => 'success', 'message' => 'Inspeksi Hydrant berhasil disimpan.']);
    }
}
