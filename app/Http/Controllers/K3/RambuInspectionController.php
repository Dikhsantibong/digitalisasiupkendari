...
class RambuInspectionController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('k3/input/rambu', [
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'rows.*.rambu' => ['nullable', 'string', 'max:255'],
        ...
        DB::transaction(function () use ($validated, $unit, $month, $year, $user): void {
            ...
            foreach ($validated['rows'] ?? [] as $index => $row) {
                ...
                if (! empty($row['id'])) {
                    $record = K3RambuInspection::query()->where('id', $row['id'])->where('unit_id', $unit->id)->first();
                    if ($record) {
                ...
                $existingIds[] = K3RambuInspection::create([...$attributes, 'unit_id' => $unit->id, 'year' => $year, 'month' => $month])->id;
            }

            K3RambuInspection::query()
                ->where('unit_id', $unit->id)->where('year', $year)->where('month', $month)
                ->when($existingIds !== [], fn ($q) => $q->whereNotIn('id', $existingIds))
                ->delete();
        });

        $this->activityLogger->log(ActivityEvent::Updated, "Menyimpan Inspeksi Rambu-Rambu K3 & B3 {$unit->name} {$month}/{$year}", $unit, unit: $unit->id);

        return back()->with('toast', ['type' => 'success', 'message' => 'Inspeksi Rambu-Rambu K3 & B3 berhasil disimpan.']);
    }
}
